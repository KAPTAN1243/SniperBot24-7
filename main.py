...
def send_webhook(username):
    kind = "Sayisal" if username.isdigit() else "Harf" if username.isalpha() else "Karma"
    embed = {
        "title": "BOSDA USERNAME BULUNDU!",
        "description": (
            "**Username:** `" + username + "`\n"
            "**Durum:** Bosda & Kullanilabilir\n"
            "**Tip:** " + kind + " (" + str(len(username)) + " karakter)\n"
            "**Tarih:** " + datetime.now().strftime('%d/%m/%Y %H:%M:%S')
        ),
        "color": 0x2ecc71,
        "footer": {"text": "Discord Sniper v6.0"},
    }
    payload = {"content": "@everyone Bosda Nick!", "embeds": [embed]}
    for _ in range(3):
        try:
